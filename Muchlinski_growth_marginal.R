...
# 
# 
# probit.plot.data <- as.data.frame(cbind(ci_probit_upper, ci_probit_lower, probit.pred$fit, test[[1]]$trade))
# names(probit.plot.data) <- c("ci_upper_bd", "ci_lower_bd", "p_hat", "trade")
# probit.plot <- create_marginal_plot_trade(probit.plot.data)


pp <- grid.arrange(btmchine.plot, dbart.plot, probit.plot, ncol = 3)
ggsave("btmachine_vs_dbart_marginal_gdpgrowth.pdf", pp, height = 8, width = 15)

# ggsave("probit_marginal_gdpgrowth.pdf", probit.plot, height = 6, width = 6)
