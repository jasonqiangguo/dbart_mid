...
save(md, file=paste0(data.path, "/final_model_10cv.RData"))
...
pd_plot_prob_25pred = function(bart_machine, j, levs = c(0.05, seq(from = 0.10, to = 0.90, by = 0.10), 0.95), lower_ci = 0.025, upper_ci = 0.975, prop_data = 1){
  ...
  n_pd_plot = round(quantile(1:bart_machine$n, probs = 0.25))
  ...
  for (q in 1 : length(x_j_quants)){
    ...
    top25 <- 1:n_pd_plot
    indices <- order(bart_machine$p_hat_train)[top25]
...
pdf(paste0(script.path, "/partial_growth_top25_10cv.pdf"))
